...
const prog_info_t _prog_info_record __attribute__((section(".hachi.prog_info"))) __attribute__((aligned(4096))) = {0};
    ...
    while (true) {
        ...
        xm_result_t res;
        {
            ...
            uint32_t pkt_timeout =
                prog_valid ? HACHI_CONFIG_XMODEM_BOOT_TIMEOUT : HACHI_CONFIG_XMODEM_INITIAL_PACKET_TIMEOUT;
        ...
        while (true) {
            if (XM_RES_OK == res) {
                ...
                if (s->num_pkts_recv < PACKETS_PER_BLOCK) {
                    res = xm_recv_packet(buf + XM_PAYLOAD_SIZE * s->num_pkts_recv, s->next_pkt_num,
                                         HACHI_CONFIG_XMODEM_PACKET_TIMEOUT, HACHI_CONFIG_XMODEM_DATA_TIMEOUT);
                    continue;
                }
                ...
                s->num_blks_recv++;
                s->num_pkts_recv = 0;
                res = xm_recv_packet(buf, s->next_pkt_num, HACHI_CONFIG_XMODEM_PACKET_TIMEOUT,
